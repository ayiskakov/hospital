...
func (s *Storage) ExecTX(ctx context.Context, options pgx.TxOptions, fn func(*Queries) error) error {
	...
	if err != nil {
		return errors.Wrap(err, "[storage.ExecTX] failed to begin transaction")
	}
	...
	if err != nil {
		...
		return errors.Wrap(err, "[storage.ExecTX] tx error occured")
	}

	if err = tx.Commit(ctx); err != nil {
		return errors.Wrap(err, "[storage.ExecTX] tx error occured while commit")
	}
